...
async function loadData() {
  ...
  (registrations.data || []).forEach((record) => {
    ...
    state.register[record.game_date][record.player_id] = { registered: Boolean(record.registered), selected: Boolean(record.selected), attended: Boolean(record.attended), paid: Boolean(record.paid), amount: record.amount };
  });
  render();
}
...
function renderSummary() {
  ...
  const metrics = [['Available', records.filter((record) => record.registered).length], ['Selected', records.filter((record) => record.selected).length], ['Attended', records.filter((record) => record.attended).length], ['Paid', records.filter((record) => record.paid).length], ['Collected', formatCurrency(records.reduce((total, record) => total + (Number(record.amount) || 0), 0))]];
...
$('#players-list').addEventListener('click', async (event) => { const summary = event.target.closest('[data-toggle-player]'); if (summary) { const id = summary.dataset.togglePlayer; expandedPlayers.has(id) ? expandedPlayers.delete(id) : expandedPlayers.add(id); renderPlayers(); return; } const edit = event.target.closest('[data-edit-player]'); if (edit) { openPlayerDialog(state.players.find((player) => player.id === edit.dataset.editPlayer)); return; } const id = event.target.dataset.deletePlayer; if (!id || !confirm('Remove this player and their historical records?')) return; const { error } = await db.from('players').delete().eq('id', id); if (error) return alert(`Could not remove player: ${error.message}`); await loadData(); });
