...
function renderPlayers() {
  ...
  list.innerHTML = state.players.map((player) => {
    ...
    const rows = history.length ? history.sort((a, b) => b.date.localeCompare(a.date)).map((record) => `<li><span class="history-date">${formatDate(record.date)}</span><span class="history-status"><b class="${record.registered ? 'is-yes' : ''}">${record.registered ? 'Registered' : 'Not registered'}</b><b class="${record.attended ? 'is-yes' : ''}">${record.attended ? 'Attended' : 'Not attended'}</b></span><strong>${record.paid ? `Paid ${formatCurrency(record.amount)}` : 'Not paid'}</strong></li>`).join('') : '<li class="payment-empty">No registration, attendance or payment records yet.</li>';
...
async function saveRoster(input) { if (!selectedDate) return; const old = state.register[selectedDate]?.[input.dataset.player] || { registered: false, attended: false, paid: false, amount: null }; if (input.dataset.field === 'paid' && input.checked && !isAmount(old.amount)) { alert('Enter a dollar amount before marking this player as paid.'); input.checked = false; return; } if (input.dataset.field === 'amount' && input.value === '' && old.paid) { alert('A paid player must have a dollar amount.'); input.value = Number(old.amount).toFixed(2); return; } const value = input.type === 'checkbox' ? input.checked : (input.value === '' ? null : Number(input.value).toFixed(2)); const { error } = await db.from('registrations').upsert({ game_date: selectedDate, player_id: input.dataset.player, ...old, [input.dataset.field]: value }); if (error) return alert(`Could not save register: ${error.message}`); await loadData(); }
